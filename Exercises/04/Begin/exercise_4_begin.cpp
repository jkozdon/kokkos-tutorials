...
//   Add custom RangePolicy to Kokkos::parallel_*** calls.
//   Experiment by changing the Layouts and Spaces; then test the effect on
//   performance.
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < argc; i++) {
    if ((strcmp(argv[i], "-N") == 0) || (strcmp(argv[i], "-Rows") == 0)) {
      ...
      printf(
          "  -Rows (-N) <int>:      exponent num, determines number of rows "
          "2^num (default: 2^12 = 4096)\n");
      printf(
          "  -Columns (-M) <int>:   exponent num, determines number of columns "
          "2^num (default: 2^10 = 1024)\n");
      printf(
          "  -Size (-S) <int>:      exponent num, determines total matrix size "
